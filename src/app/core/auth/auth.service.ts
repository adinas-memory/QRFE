  ...
  refreshUserContext() {
    return this.http.post<UserContextModel>(`${this.apiUrl}/api/user/refresh-token`, {}, { withCredentials: true }).pipe(
      tap(user => {
        ...
        console.warn('DBG909afb refresh-token success', { hasUser: !!user, role: user?.role ?? null, restaurantId: user?.restaurantId ?? null });
        // #endregion
        this.setUser(user);
      }),
      catchError(err => {
        ...
        console.warn('DBG909afb refresh-token failed', { status: err?.status ?? null, url: err?.url ?? null });
        // #endregion
